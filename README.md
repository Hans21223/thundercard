...
- **Ctrl+S** saves the card (or tree) into the Library. **Auto-save** (Settings) never saves while you type: when you switch card or tree, open the Library or leave the page, unsaved edits go into the save you opened, or into the Library's Autosave folder. Saves live in the browser's IndexedDB (hundreds of MB, not localStorage's 5 MB); the Library searches and shows pictures.
